...
export default function Home() {
  const correctCode = process.env.NEXT_PUBLIC_PASSCODE || 'trail2025';
  ...
  // Gate
  useEffect(() => {
    if (localStorage.getItem('trail-unlocked') === correctCode) {
      setUnlocked(true);
    }
  }, []);
  ...
  const checkCode = () => {
    if (attempt === correctCode) {
      localStorage.setItem('trail-unlocked', attempt);
      setUnlocked(true);
    } else {
  ...
  if (!unlocked) {
    return (
      <div className="h-screen flex flex-col items-center justify-center bg-gray-900 text-white">
        <h2 className="mb-4 text-xl">Enter passcode to continue:</h2>
        <input
          type="password"
          value={attempt}
